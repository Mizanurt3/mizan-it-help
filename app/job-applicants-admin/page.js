  ...
  return (
    <div style={{ padding: "20px", maxWidth: "1450px", margin: "0 auto", fontFamily: "system-ui, sans-serif" }}>
      ...
      <form onSubmit={handleSave} style={{ background: "#f9f9f9", padding: "35px", borderRadius: "12px", boxShadow: "0 4px 25px rgba(0,0,0,0.1)" }}>
        ...
        {/* Present & Permanent Address (same as before) */}
        <div style={{ marginBottom: "40px" }}>
        ...
        <div style={{ marginBottom: "40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "16px", marginTop: "18px" }}>
            <select value={form.ssc_exam} onChange={e => setForm({ ...form, ssc_exam: e.target.value })}>
              <option value="">Select Exam</option>{sscExams.map(ex => <option key={ex} value={ex}>{ex}</option>)}
            </select>
            <select value={form.ssc_board} onChange={e => setForm({ ...form, ssc_board: e.target.value })}>
              <option value="">Select Board</option>{boards.map(b => <option key={b} value={b}>{b}</option>)}
            </select>
            <select value={form.ssc_group} onChange={e => setForm({ ...form, ssc_group: e.target.value })}>
              <option value="">Select Group</option>{groups.map(g => <option key={g} value={g}>{g}</option>)}
            </select>
            <select value={form.ssc_result_type} onChange={e => setForm({ ...form, ssc_result_type: e.target.value })}>
              <option value="">Result Type</option>{resultTypes.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
            <select value={form.ssc_year} onChange={e => setForm({ ...form, ssc_year: e.target.value })}>
              <option value="">Passing Year</option>{years.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
        ...
        <div style={{ marginBottom: "40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "16px", marginTop: "18px" }}>
            ...
            <select value={form.hsc_group} onChange={e => setForm({ ...form, hsc_group: e.target.value })}>
              <option value="">Select Group</option>{groups.map(g => <option key={g} value={g}>{g}</option>)}
            </select>
