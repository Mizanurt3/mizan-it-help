  ...
  try {
    ...
    const result = await pool.query(`
      ...
      ON CONFLICT (mobile) DO UPDATE SET
        ...
        religion = EXCLUDED.religion, gender = EXCLUDED.gender,
        marital_status = EXCLUDED.marital_status, spouse_name = EXCLUDED.spouse_name,
        email = EXCLUDED.email, quota = EXCLUDED.quota, dep_status = EXCLUDED.dep_status,
        nid = EXCLUDED.nid, breg = EXCLUDED.breg, passport = EXCLUDED.passport, dob = EXCLUDED.dob,
        ...
        hsc_exam = EXCLUDED.hsc_exam, /* ... বাকি সব ফিল্ড একইভাবে ... */
        updated_at = CURRENT_TIMESTAMP
      RETURNING *
    `, [mobile, name, name_bn, father, father_bn, mother, mother_bn, religion, gender, marital_status, spouse_name, email, quota, dep_status, nid, breg, passport, dob, present_careof, present_village, present_district, present_post, present_postcode, present_upazila, permanent_careof, permanent_village, permanent_district, permanent_post, permanent_postcode, permanent_upazila, ssc_exam, ssc_board, ssc_roll, ssc_year, ssc_group, ssc_result_type, ssc_result, hsc_exam, hsc_board, hsc_roll, hsc_year, hsc_group, hsc_result_type, hsc_result, gra_exam, gra_subject, gra_institute, gra_year, gra_duration, gra_result_type, gra_result, mas_exam, mas_subject, mas_institute, mas_year, mas_duration, mas_result_type, mas_result]);
    ...
    console.error("=== SAVE API FULL ERROR ===", error);
    return Response.json({ 
      status: "error", 
      message: error.message || "Failed to save data. Check console for details." 
    }, { status: 500 });
  }
}